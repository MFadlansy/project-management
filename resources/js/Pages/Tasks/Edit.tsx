...
interface EditTaskProps {
    ...
    task: TaskData;   // Tugas dikirim sebagai props dari controller
}

export default function EditTask({ project, task: initialTask }: EditTaskProps) {
    const { toast } = useToast();
    const [form, setForm] = useState({
        title: initialTask.title,
        description: initialTask.description || '',
        status: initialTask.status, // Ini seharusnya sudah benar jika initialTask.status memang union type
        assigned_to: initialTask.assigned_to ? String(initialTask.assigned_to) : '',
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(route('tasks.update', { project: project.id, task: initialTask.id }), {
                method: 'PUT',
                headers: {
        ...
        <AuthenticatedLayout
            header={
                <h2 className="font-semibold text-xl text-gray-800 leading-tight">
                    Edit Tugas: {initialTask.title}
                </h2>
            }
        >
            <Head title={`Edit Tugas - ${initialTask.title}`} />

            <div className="py-12">
                <div className="max-w-xl mx-auto sm:px-6 lg:px-8 bg-white p-6 rounded-lg shadow-md">
                    <h2 className="text-2xl font-bold mb-6 text-gray-800">Edit Tugas</h2>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex justify-end gap-3 mt-6">
                            ...
                            <Button type="submit" disabled={loading}>
                                {loading ? 'Memperbarui...' : 'Perbarui Tugas'}
                            </Button>
                        </div>
                    </form>
                </div>
